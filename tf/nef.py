...
tf.app.flags.DEFINE_string("model", "ef_single_state",
                           "Model for training: quetch or ef_single_state")
...
tf.app.flags.DEFINE_string("sketch_dir", "sketches/",
...
tf.app.flags.DEFINE_boolean("concat", True,
...
tf.app.flags.DEFINE_integer("lstm_units", 20,
                            "number of LSTM-RNN encoder units")
...
tf.app.flags.DEFINE_float("keep_prob", 1,
                          "keep probability for dropout during training (1: no dropout)")
tf.app.flags.DEFINE_float("keep_prob_sketch", 1,
                          "keep probability for dropout during sketching (1: no dropout)")
...
tf.app.flags.DEFINE_integer("sketch_sentence_id", 434,
                            "sentence id of sample (dev) to keep track of during sketching")
FLAGS = tf.app.flags.FLAGS

logging.basicConfig(
    filename=FLAGS.model_dir + str(datetime.datetime.now()).replace(" ",
                                                                    "-") + ".training.log")
...
class EasyFirstModel():
    ...
    def __init__(self, K, D, N, J, r, src_vocab_size, tgt_vocab_size,
                 batch_size, optimizer, learning_rate,
                 max_gradient_norm, lstm_units, concat, buckets, window_size,
                 src_embeddings,
                 ...
                 attention_discount_factor=0.0, attention_temperature=1.0):
        """
        ...
        if self.lstm_units > 0:
            if self.bilstm:
                logger.info(
                    "Model with bi-directional LSTM RNN encoder of %d units" % self.lstm_units)
            else:
                logger.info(
                    "Model with uni-directional LSTM RNN encoder of %d units" % self.lstm_units)
        else:
            if self.src_embeddings.table is None and self.tgt_embeddings.table is None:
                ...
                logger.info(
                    "Model with simple embeddings of size %d (src) & %d (tgt)" % \
                    (self.src_embeddings.table.shape[0],
                     self.tgt_embeddings.table.shape[0]))
        ...
        if self.keep_prob_sketch < 1:
            logger.info(
                "Dropout during sketching with p=%f" % self.keep_prob_sketch)
        ...
        for j, max_len in enumerate(self.buckets):
            ...
            self.seq_lens.append(tf.placeholder(tf.int64,
                                                shape=[None],
                                                name="seq_lens{0}".format(j)))
            self.keep_prob_sketches.append(tf.placeholder(tf.float32,
                                                          name="keep_prob_sketch{0}".format(
                                                              j)))
            ...
            with tf.variable_scope(tf.get_variable_scope(),
                ...
                logger.info(
                    "Initializing parameters for bucket with max len %d" % max_len)
                bucket_losses, bucket_losses_reg, bucket_predictions, src_table, tgt_table, sketches = model_func(
                    inputs=self.inputs[j], labels=self.labels[j],
                    masks=self.masks[j],
                    seq_lens=self.seq_lens[j],
                    src_vocab_size=self.src_vocab_size,
                    tgt_vocab_size=self.tgt_vocab_size, K=self.K,
                    D=self.D, N=max_len,
                    # as many sketches as words in sequence
                    ...
                    keep_prob=self.keep_probs[j],
                    keep_prob_sketch=self.keep_prob_sketches[j],
                    ...
                    is_train=self.is_trains[j])

                self.losses_reg.append(bucket_losses_reg)
                self.losses.append(
                    bucket_losses)  # list of tensors, one for each bucket
                self.predictions.append(
                    bucket_predictions)  # list of tensors, one for each bucket
    ...
    def batch_update(self, session, bucket_id, inputs, labels, masks, seq_lens,
                     forward_only=False):
        """
        Training step
        :param session:
        ...
        input_feed[self.keep_probs[
            bucket_id].name] = 1 if forward_only else self.keep_prob
        input_feed[self.keep_prob_sketches[
            bucket_id].name] = 1 if forward_only else self.keep_prob_sketch
        ...
        return outputs[0], predictions, outputs[
            2]  # loss, predictions, regularized loss

    def get_sketches_for_single_sample(self, session, bucket_id, input, label,
                                       mask, seq_len):
        """
        fetch the sketches and the attention for a single sample from the graph
        """
        input_feed = {}
        input_feed[self.inputs[bucket_id].name] = np.expand_dims(input,
                                                                 0)  # batch_size = 1
...
def create_model(session, buckets, src_vocab_size, tgt_vocab_size,
    ...
    model = EasyFirstModel(K=FLAGS.K, D=FLAGS.D, N=FLAGS.N, J=FLAGS.J,
                           r=FLAGS.r,
                           ...
                           update_emb=FLAGS.update_emb,
                           model=FLAGS.model, activation=FLAGS.activation,
    ...
    if checkpoint and tf.gfile.Exists(
            checkpoint.model_checkpoint_path) and FLAGS.restore:
        logger.info(
            "Reading model parameters from %s" % checkpoint.model_checkpoint_path)
...
def train():
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        train_src_embeddings, train_tgt_embeddings = load_data(train_dir,
                                                               src_embeddings,
                                                               tgt_embeddings,
        ...
        dev_feature_vectors, dev_tgt_sentences, dev_labels, dev_label_dict = \
            load_data(dev_dir, train_src_embeddings, train_tgt_embeddings,
                      train=False,
                      labeled=True)  # use training vocab for dev

        if FLAGS.src_embeddings == "":
            src_embeddings = embedding.Embedding(None,
                                                 train_src_embeddings.word2id,
        ...
        if FLAGS.tgt_embeddings == "":
            tgt_embeddings = embedding.Embedding(None,
                                                 train_tgt_embeddings.word2id,
        ...
        logger.info("Maximum sentence length (train): %d" % max(
            [len(y) for y in train_labels]))
        logger.info("Maximum sentence length (dev): %d" % max(
            [len(y) for y in dev_labels]))
        ...
        # bucketing dev data
        dev_buckets, dev_reordering_indexes = put_in_buckets(
            np.asarray(dev_feature_vectors),
            np.asarray(dev_labels),
            buckets=bucket_edges)

        # create the model
        model = create_model(sess, bucket_edges, src_vocab_size, tgt_vocab_size,
                             False, src_embeddings, tgt_embeddings,
                             class_weights)

        train_buckets_sizes = {i: len(indx) for i, indx in
                               train_reordering_indexes.items()}
        dev_buckets_sizes = {i: len(indx) for i, indx in
                             dev_reordering_indexes.items()}

        logger.info("Creating buckets for training data:")
        for i in train_buckets.keys():
            X_train_padded, Y_train_padded, train_masks, train_seq_lens = \
                train_buckets[i]
            total_number_of_pads = sum(
                [bucket_edges[i] - l for l in train_seq_lens])
            logger.info(
                "Bucket no %d with max length %d: %d instances, avg length %f,  " \
                "%d number of PADS in total" % (
                    i, bucket_edges[i], train_buckets_sizes[i],
        ...
        for i in dev_buckets.keys():
            ...
            total_number_of_pads = sum(
                [bucket_edges[i] - l for l in dev_seq_lens])
            logger.info(
                "Bucket no %d with max length %d: %d instances, avg length %f,  " \
                "%d number of PADS in total" % (
                    i, bucket_edges[i], dev_buckets_sizes[i],
                    np.average(dev_seq_lens), total_number_of_pads))

        # choose a training sample to analyse during sketching
        if FLAGS.track_sketches:
            dev_corpus_id = FLAGS.sketch_sentence_id
            sample_bucket_id = dev_reordering_indexes.keys()[np.nonzero(
                [dev_corpus_id in dev_reordering_indexes[b] for b in
                 dev_reordering_indexes.keys()])[0][0]]
            print dev_reordering_indexes
            print sample_bucket_id
            sample_in_bucket_index = np.nonzero([i == dev_corpus_id for i in
                                                 dev_reordering_indexes[
                                                     sample_bucket_id]])[
                0]  # position of sample within bucket
            logger.info(
                "Chosen sketch sample (dev): corpus id %d, bucket %d, index in bucket %d" % (
                    dev_corpus_id, sample_bucket_id, sample_in_bucket_index))
        ...
        for epoch in xrange(FLAGS.epochs):
            ...
            for bucket_id in bucket_ids:
                bucket_xs, bucket_ys, bucket_masks, bucket_seq_lens = \
                ...
                if train_buckets_sizes[bucket_id] > FLAGS.batch_size:
                    number_of_batches = np.ceil(
                        train_buckets_sizes[bucket_id] / float(
                            FLAGS.batch_size))
                ...
                for i, batch_samples in enumerate(batch_ids):
                    ...
                    step_loss, predictions, step_loss_reg = model.batch_update(
                        sess, bucket_id,
                        x_batch, y_batch,
                        mask_batch,
                        seq_lens_batch,
                    ...
                    train_true.extend(
                        y_batch)  # needs to be stored because of random order
                    current_sample += len(x_batch)

                logger.info(
                    "bucket %d - loss %0.2f - loss+reg %0.2f" % (bucket_id,
                                                                 bucket_loss / len(
                                                                     bucket_xs),
                                                                 bucket_loss_reg / len(
                                                                     bucket_xs)))
            ...
            logger.info(
                "EPOCH %d: epoch time %fs, loss %f, train acc. %f, f1 prod %f (%f/%f) " % \
                (
            ...
            for bucket_id in dev_buckets.keys():
                bucket_xs, bucket_ys, bucket_masks, bucket_seq_lens = \
                    dev_buckets[bucket_id]
                step_loss, predictions, step_loss_reg = model.batch_update(sess,
                                                                           bucket_id,
                                                                           bucket_xs,
                                                                           bucket_ys,
                                                                           bucket_masks,
                ...
                if FLAGS.model == "ef_single_state" and FLAGS.track_sketches:
                    if bucket_id == sample_bucket_id:
                        all_sketches = model.get_sketches_for_single_sample(
                            sess, bucket_id, bucket_xs[sample_in_bucket_index][0],
                            bucket_ys[sample_in_bucket_index][0],
                            bucket_masks[sample_in_bucket_index][0],
                            bucket_seq_lens[sample_in_bucket_index][0])
                        sample_sketch = np.squeeze(all_sketches, 1)
                        print bucket_xs[sample_in_bucket_index]
                        print bucket_ys[sample_in_bucket_index]
                        print bucket_seq_lens[sample_in_bucket_index]
                        logger.info("sketch sample true: %s",
                                    str(bucket_ys[sample_in_bucket_index][0]))
                        logger.info("sketch sample predicted: %s",
                                    str(predictions[sample_in_bucket_index]))
                        pkl.dump(sample_sketch, open(
                            "%s/sketches_sent%d_epoch%d.pkl" % (
                                FLAGS.sketch_dir, dev_corpus_id, epoch), "wb"))
            ...
            logger.info(
                "EPOCH %d: validation time %fs, loss %f, dev acc. %f, f1 prod %f (%f/%f) " % \
                (
        ...
        logger.info(
            "Training finished after %d epochs. Best validation result: %f at epoch %d." \
...
def test():
    ...
    with tf.Session() as sess:
        ...
        model = create_model(sess, src_vocab_size=src_vocab_size,
                             tgt_vocab_size=tgt_vocab_size, buckets=None,
                             forward_only=True, src_embeddings=src_embeddings,
                             tgt_embeddings=tgt_embeddings,
                             class_weights=class_weights)

        # bucketing test data
        bucket_edges = model.buckets
        test_buckets, test_reordering_indexes = put_in_buckets(
            np.asarray(test_feature_vectors),
            np.asarray(test_labels),
            buckets=bucket_edges)
        test_buckets_sizes = {i: len(indx) for i, indx in
                              test_reordering_indexes.items()}

        logger.info("Creating buckets for test data:")
        for i in test_buckets.keys():
            X_test_padded, Y_test_padded, test_masks, test_seq_lens = \
                test_buckets[i]
            total_number_of_pads = sum(
                [bucket_edges[i] - l for l in test_seq_lens])
            logger.info(
                "Bucket no %d with max length %d: %d instances, avg length %f,  " \
                "%d number of PADS in total" % (
                    i, bucket_edges[i], test_buckets_sizes[i],
        ...
        for bucket_id in test_buckets.keys():
            bucket_xs, bucket_ys, bucket_masks, bucket_seq_lens = test_buckets[
                bucket_id]
            step_loss, predictions, step_loss_reg = model.batch_update(sess,
                                                                       bucket_id,
                                                                       bucket_xs,
            ...
            for i, p in enumerate(predictions):
                predictions_with_id[
                    sample_indices[i]] = p  # get back the order of the inputs
...
def demo():
    ...
    with tf.Session() as sess:
        ...
        model = create_model(sess, src_vocab_size=src_vocab_size,
                             tgt_vocab_size=tgt_vocab_size,
                             buckets=None, forward_only=True,
                             src_embeddings=src_embeddings,
                             tgt_embeddings=tgt_embeddings,
                             class_weights=class_weights)
        ...
        while sentence:
            ...
            # get word ids
            words_src = [src_embeddings.get_id(src_word) for src_word in
                         src.split()]
            words_tgt = [tgt_embeddings.get_id(tgt_word) for tgt_word in
                         tgt.split()]
            ...
            for i, w in enumerate(words_tgt):
                ...
                for j in range(i - context_size,
                               i + context_size + 1):  # source context words
                ...
                for j in range(i - context_size,
                               i + context_size + 1):  # target context words
            ...
            while len(outputs) < len(
                    words_tgt):  # can happen because of sentence length limit
                outputs.append(0)
            print "prediction: ", zip(tgt.split(), outputs)
            sys.stdout.flush()
            sys.stdout.write(
                "Enter source and target sentence separated by '|'.\n")
            sys.stdout.write("> ")
            sys.stdout.flush()
            sentence = sys.stdin.readline()
