...
class LabelAlphabet : public std::unordered_map<std::string, int> {
 public:
  LabelAlphabet() {}
  virtual ~LabelAlphabet() {}

  int GetNumLabels() const { return size(); }
  ...
  int InsertLabel(const std::string &label) {
    int lid;
    std::unordered_map<std::string, int>::iterator it = find(label);
    if (it != end()) {
      lid = it->second;
    } else {
      lid = size();
