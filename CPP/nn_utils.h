...
// Computes a columnwise log-sum-exp, returning a vector.
template<typename Real>
void LogSumExp(const Matrix<Real> &X, Vector<Real> *result) {
  //std::cout << X.rows() << " " << X.cols() << std::endl;
  Vector<Real> xmax = X.colwise().maxCoeff();
  //std::cout << xmax.rows() << " " << xmax.cols() << std::endl;
  *result = xmax + (X.rowwise() - xmax.transpose()).array().exp().colwise().
    sum().log().matrix().transpose();
}
